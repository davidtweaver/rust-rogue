extern crate specs;
use specs::prelude::*;
use super::{CombatStats, CanMelee, Name, SufferDamage};
...
impl<'a> System<'a> for MeleeCombatSystem {
    ...
    fn run(&mut self, data : Self::SystemData) {
        let (entities, mut can_melee, names, combat_stats, mut inflict_damage) = data;

        for (_entity, can_melee, name, stats) in (&entities, &can_melee, &names, &combat_stats).join() {
            if stats.hp > 0 {
